...
static bool handle_super(struct server *server, xkb_keysym_t sym) {
	switch (sym) {
	case XKB_KEY_Escape:
...
static bool handle_alt(struct server *server, xkb_keysym_t sym) {
	switch (sym) {
	case XKB_KEY_Tab:
	...
	if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
		if (modifiers & WLR_MODIFIER_LOGO) {
			for (int i = 0; i < nsyms; i++) {
				handled = handle_super(server, syms[i]);
			}
		}
		else if (modifiers & WLR_MODIFIER_ALT) {
			for (int i = 0; i < nsyms; i++) {
				handled = handle_alt(server, syms[i]);
			}
		}
	}
...
void handle_new_pointer(struct server *server,
		struct wlr_input_device *device) {
